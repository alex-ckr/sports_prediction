...
# ————————————————————————————————————————————————
# Model (mirrors model.js / mlb_model_v4.py)
# ————————————————————————————————————————————————
import math

K = dict(REGRESSION_GAMES=70, LEAGUE_RATE=4.10, RUNS_PER_WIN=10.0,
         HFA_ODDS=1.19, LOGODDS_PER_WIN=4.0, DEFAULT_SP_IP=5.5,
         FATIGUE_FREE_IP3=9.0, FATIGUE_RUNS_PER_IP=0.12, WOBA_TO_RUNS=1.15,
         PA_VS_STARTER=26, PITCHES_VS_STARTER=90)
...
def predict(home, away):
    def wpct(s):
        w, l = _n(s.get("wins"), 0), _n(s.get("losses"), 0)
        return (w + 0.5 * K["REGRESSION_GAMES"]) / (w + l + K["REGRESSION_GAMES"])

    def pitch(s):
        sp_ip = min(max(_n(s.get("sp_ip"), K["DEFAULT_SP_IP"]), 0), 9)
        bp = _n(s.get("bp_rate"), K["LEAGUE_RATE"])
        ip3 = s.get("bp_ip3")
        if ip3 is not None and ip3 > K["FATIGUE_FREE_IP3"]:
            bp += (ip3 - K["FATIGUE_FREE_IP3"]) * K["FATIGUE_RUNS_PER_IP"]
        sp = _n(s.get("sp_rate"), K["LEAGUE_RATE"])
        return ((sp - K["LEAGUE_RATE"]) * sp_ip
                + (bp - K["LEAGUE_RATE"]) * (9 - sp_ip)) / 9

    def off(s):
        r = 0.0
        vh, ov = s.get("woba_vs_hand"), s.get("woba_overall")
        if vh is not None and ov is not None:
            r += (vh - ov) / K["WOBA_TO_RUNS"] * K["PA_VS_STARTER"]
        r += _n(s.get("ptype_rv100"), 0) * K["PITCHES_VS_STARTER"] / 100
        return r

    a, b = wpct(home), wpct(away)
    p0 = (a * (1 - b)) / (a * (1 - b) + b * (1 - a))
    logit = math.log(p0 / (1 - p0)) + math.log(K["HFA_ODDS"])
    delta = pitch(away) - pitch(home) + off(home) - off(away)
    logit += delta / K["RUNS_PER_WIN"] * K["LOGODDS_PER_WIN"]
    return 1 / (1 + math.exp(-logit)), delta
